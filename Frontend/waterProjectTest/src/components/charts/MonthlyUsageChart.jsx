...
export default function MonthlyUsageChart() {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const res = await api.get("/household-user/charts/monthly-usage");
  ...
  return (
    <ResponsiveContainer width="100%" height={260}>
      <AreaChart data={data} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
        ...
        <XAxis dataKey="month" tick={{ fontSize: 11, fill: theme.colors.textMuted }} axisLine={{ stroke: theme.colors.border }} tickLine={false} />
        ...
        <Tooltip content={<ChartTooltip unit=" L" />} cursor={{ stroke: theme.colors.accent, strokeWidth: 1, strokeDasharray: "4 4" }} />
        <Area
          type="monotone" dataKey="usage" stroke={theme.colors.accent} strokeWidth={2.5}
          fill="url(#monthlyUsageGradient)" dot={{ r: 3, fill: theme.colors.accent, strokeWidth: 0 }}
